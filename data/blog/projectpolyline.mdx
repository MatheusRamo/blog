...
para utilizá-la como Breakline na superfície. Para desenvolver esse comando, segui os seguintes passos:

1. Seleciono uma Polyline existente.
2. Escolho o Profile View.
...
7. Percorro os PVI (Pontos de Interesse Vertical) do profile criado.
8. Adiciono as coordenadas e elevações dos PVI à Polyline 3D.
9. Adiciono a Polyline 3D ao Model.

Abaixo irei explicar com código cada etapa:

1. Selecionei uma Polyline existente.
...
```
4. Extrai as informações de geometria da Polyline e adiciono-as ao Profile vazio.
```csharp
...
```
8. Adiciono a Polyline 3D ao Model e removi o Profile criado.
```csharp
